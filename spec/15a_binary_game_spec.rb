...
describe BinaryGame do
  ...
  describe '#player_input' do
    ...
    context 'when user inputs two incorrect values, then a valid input' do
      before do
        invalid_input = '-9'
        other_invalid_input = 'bad_apple'
        valid_input = '3'
        allow(game_input).to receive(:gets).and_return(invalid_input, other_invalid_input, valid_input)
      end
  ...
  describe '#verify_input' do
    ...
    context 'when given a valid input as argument' do
      it 'returns valid input' do
        ...
        expect(verified_input).to be(valid_input)
      end
    end
  ...
  describe '#display_turn_order' do
    ...
    before do
      ...
      # Without this first method stub, I found that I received puts and print statements in my test output.
      allow($stdout).to receive(:write)
      allow(binary_search_turn).to receive(:min).and_return(3)
      allow(binary_search_turn).to receive(:max).and_return(5)
      allow(binary_search_turn).to receive(:make_guess).and_return(4)
      allow(binary_search_turn).to receive(:guess).and_return(4)
      allow(binary_search_turn).to receive(:update_range)
      # Without this last method stub, I found that these final examples were taking multiple seconds to complete.
      allow(game_display).to receive(:sleep)
    end

    # Command Method -> Test the change in the observable state
    it 'increases guess_count by one' do
      expect { game_display.display_turn_order(binary_search_turn) }.to change { game_display.instance_variable_get(:@guess_count) }.by(1)
    end
