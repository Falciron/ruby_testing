...
describe NumberGame do
  ...
  describe '#game_over?' do
    ...
    context 'when user guess is not correct' do
      subject(:mid_game) { described_class.new(9, '1') }
      # remove the 'x' before running this test
  ...
  describe '#player_turn' do
    ...
    context 'when user inputs two incorrect values, then a valid input' do
      before do
        ...
        bad_number = '-16'
        valid_input = '8'
        allow(game_loop).to receive(:player_input).and_return(letter, bad_number, valid_input)
      end
  ...
  describe '#final_message' do
    ...
    # Write a test for the following context.
    subject(:game_four_plus) { described_class.new(2, '2', 5) }
    context 'when count is 4 and over' do
      # remove the 'x' before running this test
      it 'outputs correct phrase' do
        win_phrase = "That was hard. It took you 5 guesses!\n"
        expect { game_four_plus.final_message }.to output(win_phrase).to_stdout
      end
    end
  end
end
