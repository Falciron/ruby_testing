...
describe FindNumber do
  ...
  describe '#make_guess' do
    ...
    subject(:game_guessing_five_nine) { described_class.new(5, 9, number_guessing) }
    context 'when min is 5 and max is 9' do
      it 'returns 7' do
        expect(game_guessing_five_nine.make_guess).to eq(7)
      end
    end

    subject(:game_guessing_eight_nine) { described_class.new(8, 9, number_guessing) }
    context 'when min is 8 and max is 9' do
      it 'returns 8' do
        expect(game_guessing_eight_nine.make_guess).to eq(8)
      end
    end

    subject(:game_guessing_zero_three) { described_class.new(0, 3, number_guessing) }
    context 'when min is 0 and max is 3' do
      it 'returns 1' do
        expect(game_guessing_zero_three.make_guess).to eq(1)
      end
    end

    subject(:game_guessing_three_three) { described_class.new(3, 3, number_guessing) }
    context 'when min and max both equal 3' do
      it 'returns 3' do
        expect(game_guessing_three_three.make_guess).to eq(3)
      end
    end
  end
  ...
  describe '#game_over?' do
    context 'when guess and random_number are equal' do
      ...
      let(:number_guessing) { double('random_number', value: 8) }
      subject(:game_correctly_guessed) { described_class.new(0, 9, number_guessing, 8) }
      ...
      it 'is game over' do
        expect(game_correctly_guessed).to be_game_over
      end
    end
    ...
    context 'when guess and random_number are not equal' do
      let(:number_guessing) { double('random_number', value: 8) }
      subject(:game_incorrectly_guessed) { described_class.new(0, 9, number_guessing, 4) }
      it 'is not game over' do
        expect(game_incorrectly_guessed).not_to be_game_over
      end
    end
  end
  ...
  describe '#update_range' do
    ...
    context 'when the guess is 7, min=5, and max=8' do
      ...
      it 'updates min to the same value as max' do
        expect { narrow_guess_game.update_range }.to change { narrow_guess_game.min }.to(8)
      end

      it 'does not update max' do
        expect { narrow_guess_game.update_range }.not_to(change { narrow_guess_game.max })
      end
    end
  end
end
